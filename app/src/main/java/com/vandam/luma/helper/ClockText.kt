...
fun formatClockText(
    ...
    val hour =
        when (prefs.timeFormat) {
            Prefs.TimeFormat.TwentyFourHour -> calendar.get(Calendar.HOUR_OF_DAY)
            Prefs.TimeFormat.Standard,
            Prefs.TimeFormat.TwelveHour,
            -> calendar.get(Calendar.HOUR).let { if (it == 0) 12 else it }
        }
    val minute = calendar.get(Calendar.MINUTE)
    val hourText =
        if (prefs.timeFormat == Prefs.TimeFormat.TwentyFourHour || prefs.leadingZero) {
            "%02d".format(hour)
        } else {
            hour.toString()
        }
    ...
    return buildString {
        ...
        if (prefs.timeFormat == Prefs.TimeFormat.Standard) {
            append(if (calendar.get(Calendar.AM_PM) == Calendar.AM) " AM" else " PM")
        }
