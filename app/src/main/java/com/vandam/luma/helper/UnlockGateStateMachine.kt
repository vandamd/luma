...
internal data class UnlockGateState(
    ...
    fun toUiSnapshot(): UnlockGateUiSnapshot =
        UnlockGateUiSnapshot(
            phase = phase,
            visible = phase != UnlockGatePhase.Idle && phase != UnlockGatePhase.WakeCover,
            showingHomeStatusBar =
                phase != UnlockGatePhase.Idle &&
                    phase != UnlockGatePhase.WakeCover &&
...
internal class UnlockGateStateMachine(
    ...
    private fun reduce(
        ...
        when (event) {
            ...
            is UnlockGateEvent.ScreenOff ->
                Reduction(
                    // Keep an opaque cover attached across sleep/wake so the last
                    // foreground app never gets a visible frame while the gate is rebuilt.
                    currentState.enterWakeCover(),
                )
    ...
    private fun UnlockGateState.enterWakeCover(): UnlockGateState =
        copy(
            phase = UnlockGatePhase.WakeCover,
            wakeArmed = true,
            ignoreNextHomeUp = false,
